...
@ApplicationScoped
public class XlsxPlantingSummaryReport extends AbstractXlsxReport {
  ...
  public void createReport(OutputStream output, Facility facility, Locale locale, Map<String, String> parameters) throws ReportException {
    try (XlsxBuilder xlsxBuilder = new XlsxBuilder()) {
      String title = localesController.getString(locale, "reports.planting_summary.title");
      ...
      xlsxBuilder.setCellValue(sheetId, rowIndex, amountIndex, localesController.getString(locale, "reports.planting_summary.amount"));

      Map<Product, Integer> productCounts = new LinkedHashMap<>(); // to preserve order for tests

      for (Event event : events) {
        PlantingEvent sowingEvent = (PlantingEvent) event;
        int count = productCounts.getOrDefault(sowingEvent.getProduct(), 0);

        productCounts.put(sowingEvent.getProduct(), count + sowingEvent.getGutterCount());
      }
      ...
      rowIndex++;
      xlsxBuilder.setCellValue(sheetId, rowIndex, productIndex, localesController.getString(locale, "reports.planting_summary.total"));
